 ...
 * {@link SecurityQuestionFragment.OnFragmentInteractionListener} interface
...
public class SecurityQuestionFragment extends Fragment {
    ...
    @BindView(R.id.sq_spinner)
    Spinner mSqQuestionSpinnerView;
    @BindView(R.id.sq_answer)
    EditText mSqAnswerView;
    ...
    @OnClick(R.id.sq_btn)
    public void handleSqBtn(){
        String sqQuestion = mSqQuestionSpinnerView.getSelectedItem().toString();
        String sqAnswer = mSqAnswerView.getText().toString();

        if(Util.isNull(sqQuestion))
            PDialog.showToast(getActivity(), "Something Wrong with Dropdown");
        else if(Util.isNull(sqAnswer)) {
            mSqAnswerView.setError(getString(R.string.sq_field_required));
            mSqAnswerView.requestFocus();
        }
        else {
            UserDetailModel model = new UserDetailModel();
            model.setSecurityQuestion(sqQuestion);
            model.setSecurityQuestionAnswer(sqAnswer);
            if(mListener != null)
                mListener.onCreateAccountBtnClicked(model);
        }
    }

    public void onResponseOfSecurityQuestion(List<String> values){
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this.getActivity(), android.R.layout.simple_spinner_item, values);
        adapter.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);
        mSqQuestionSpinnerView.setAdapter(adapter);
    }
    ...
    public interface OnFragmentInteractionListener {
        ...
        void onCreateAccountBtnClicked(UserDetailModel model);
    }
}
